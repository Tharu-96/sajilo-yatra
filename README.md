# 🚌 Sajilo Yatra - Cinematic Transit App
...
*   **Interactive Maps**: Powered by OpenStreetMap (OSRM & Nominatim) and Leaflet (`flutter_map`), featuring live tracking, panning, and zoom support.
...
> - Never commit your `.env` files. They are included in `.gitignore` by default.
> - Ensure your production `SECRET_KEY` and database credentials are long, randomly generated, and stored securely (e.g., via a secret manager or secure environment variables).
> - The application handles password hashing securely using `bcrypt`.
